...
下面就直接上**四道** LeetCode 原题来套这个框架，其中第一道题会详细说明其原理，后面三道就直接闭眼睛秒杀了。
...
![image.png](assets/image-20210405212407-fgjkos9.png)
...
1、我们在字符串 `S` 中使用双指针中的左右指针技巧，初始化 `left = right = 0`，把**左闭右开**区间 `[left, right)` 称为一个「窗口」。
...
![image.png](assets/image-20210405212415-y5pyybo.png)

增加 `right`，直到窗口 `[left, right]` 包含了 `T` 中所有字符：

![image.png](assets/image-20210405212425-cw813l3.png)

现在开始增加 `left`，缩小窗口 `[left, right]`。

![image.png](assets/image-20210405212432-qegecae.png)

直到窗口中的字符串不再符合要求，`left` 不再继续移动。

![image.png](assets/image-20210405212441-wd2a6l4.png)
...
![image.png](assets/image-20210405212450-idzbva0.png)
...
![image.png](assets/image-20210405212459-r63v6nm.png)
...
class Solution {
    public List<Integer> findAnagrams(String s, String p) {
        ...
        while(right < s.length()){
            ...
            while(valid == need.size()){
                if(p.length() == right-left) res.add(left);
                char rmv = s.charAt(left++);
                if(need.containsKey(rmv)){
                    if(need.get(rmv).equals(window.get(rmv))) valid--;
...
![image.png](assets/image-20210405212507-o1xzhmx.png)
...
class Solution {
    public int lengthOfLongestSubstring(String s) {
        ...
        while (right < s.length()) {
            char c = s.charAt(right++);
            window.put(c, window.getOrDefault(c,0)+1);// 进行窗口内数据的一系列更新
            while (window.get(c) > 1) { // 判断左侧窗口是否要收缩
                char d = s.charAt(left++);
                window.put(d, window.getOrDefault(d,0)-1); // 进行窗口内数据的一系列更新
            }
            res = Math.max(res, right - left);// 在这里更新答案
        }
        return res;
    }
}
    ...
    public int lengthOfLongestSubstring(String s) {
        ...
        while (right < s.length()) {
            ...
            if (temp.containsKey(c)) {
                ...
                while (left <= index)
                    temp.remove(s.charAt(left++));  //将left移到重复字符的后面，途中的字符全移出map
            }
            temp.put(c, right++);
        }
        return Math.max(res, right - left);//right到达最右还要计算一次长度
    }
}
```